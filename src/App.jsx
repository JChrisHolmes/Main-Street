        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {Object.entries(IND).map(([k, v]) => (
              <button key={k} onClick={() => toggleIndustry(k)} style={{ padding: '12px', background: industries.has(k) ? v.color : 'transparent', border: `2px solid ${industries.has(k) ? v.color : t.border}`, borderRadius: 8, color: industries.has(k) ? '#fff' : t.text, fontFamily: 'Space Mono,monospace', fontSize: 11, cursor: 'pointer', fontWeight: 700, transition: 'all 0.2s' }}>
                {v.emoji} {v.label}
              </button>
            ))}
          </div>
